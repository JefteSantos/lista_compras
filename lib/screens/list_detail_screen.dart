...
class _ListDetailScreenState extends State<ListDetailScreen> {
  ...
  void _editarNomeLista(BuildContext context, ListaCompras lista) async {
    ...
    if (novoNome != null && novoNome.isNotEmpty && novoNome != lista.nome) {
      lista.nome = novoNome;
      if (mounted) {
         Provider.of<ListasProvider>(context, listen: false).atualizarLista(lista);
      }
    }
  }

  void _adicionarOuEditarItem(BuildContext context, ListaCompras lista, {Item? item}) async {
  ...
  void _confirmarExclusaoItem(BuildContext context, ListaCompras lista, Item item) async {
    final confirmed = await showGenericConfirmationDialog(
      context,
      title: 'Excluir Item',
      content: 'Deseja excluir "${item.nome}"?',
      confirmText: 'EXCLUIR',
      confirmColor: Colors.red,
    );

    if (confirmed && mounted) {
      Provider.of<ListasProvider>(context, listen: false).removerItem(lista, item);
    }
  }
  ...
  void _copiarParaAreaTransferencia(ListaCompras lista) {
    ...
    if (itensPendentes.isNotEmpty) {
      ...
      for (var item in itensPendentes) {
        final preco = item.preco != null ? ' (${AppUtils.formatMoney(item.preco!)})' : '';
    ...
    buffer.writeln('\n📊 *Resumo:*');
    if (lista.precoComprado > 0) buffer.writeln('✅ Carrinho: ${AppUtils.formatMoney(lista.precoComprado)}');
    if (totalFaltando > 0) buffer.writeln('⬜ Falta: ${AppUtils.formatMoney(totalFaltando)}');
    buffer.writeln('💰 *Total: ${AppUtils.formatMoney(lista.precoTotal)}*');
    
    Clipboard.setData(ClipboardData(text: buffer.toString()));
    ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Lista copiada com sucesso! Pronta para colar no WhatsApp.'))
    );
  }
        ...
        return Scaffold(
          appBar: AppBar(
            title: GestureDetector(
                ...
                child: Row(
                  children: [
                    Expanded(child: Text(lista.nome, overflow: TextOverflow.ellipsis)),
                    ...
                    ? Center(
                        child: Column(
                            ...
                            children: [
                                ...
                                const Text('Lista vazia. Adicione itens!', style: TextStyle(color: Colors.grey)),
                            ],
                        ),
                    )
                    : ListView.separated(
                      ...
                      children: [
                          ...
                          ElevatedButton.icon(
                              onPressed: () {
                                  ...
                                  Navigator.pop(context); // Volta pra home pois a lista mudou de tab (provavelmente)
                              },
                              icon: Icon(lista.finalizada ? Icons.replay : Icons.check),
                              label: Text(lista.finalizada ? 'Reabrir Lista' : 'Finalizar Compra'),
                              style: ElevatedButton.styleFrom(
                                  backgroundColor: lista.finalizada ? Colors.orange : Colors.green,
  ...
  Widget _buildItemTile(BuildContext context, ListasProvider provider, ListaCompras lista, Item item) {
      return Dismissible(
          key: Key(item.id),
                  ...
                  children: [
                       Text(
                          '${item.quantidade} un${item.preco != null ? ' x ${AppUtils.formatMoney(item.preco!)}' : ''}',
                           style: TextStyle(color: item.comprado ? Colors.grey.shade400 : Colors.grey.shade700),
                       ),
